...
const Config: FC = () => {
    const [formData, updateFormData] = useState<ConfigState>({
        googlecalendarids: '',
        googleapikey: '',
        darkskyapikey: '',
        latlong: '',
    ...
    useEffect(() => {
        Object.keys(formData).forEach(key => {
            updateFormData({ ...formData, [key]: localStorage.getItem(key) });
        });
    }, [formData]);
                    ...
                    <fieldset key={key}>
                        ...
                        {_getKeyByName(configFields, k).data.map((e: any, key2: number) => (
                            <div key={key2}>
                                ...
                                <input type={_getKeyByName(configFields, k).type === 'password' && !showSecrets ? 'password' : 'text'}
                                    name={e.name} value={_getKeyByName(formData, e.name)} onChange={handleChange} />
                            </div>
                        ))}
                    </fieldset>
                ))}
